...
def main():
    ...
    for char, count in distribution_after.items():
        print("{:<6} {:<6}".format(char, count))
    print(f'amount of char in input: {count_char(input_rtf_file)}')
    print(f'amount of char in message: {count_char(message_file)}')
    print(f'amount of char in result: {count_char(output_rtf_file)}')
